...
class TrainingPresencesController < ApplicationController
  def show
    @player = User.find params.expect(:user_id)
    @training = Training.find params.expect(:training_id)
  ...
  def create
    current_user.present_for! Training.find(params.expect(:training_id))
    redirect_with(fallback: root_path)
  end

  def destroy
    current_user.not_present_for! Training.find(params.expect(:training_id))
    redirect_with(fallback: root_path)
  end
