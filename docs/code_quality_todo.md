...
- **Status**: in_progress — strategy chosen: scoped finders (not Pundit), delivered as separate PRs per controller family. PR 1 (global `verify_user_member_of_section` gate + shared request-spec example): branch `security/section-membership-gate` — merged (#1100). PR 2 (scope `Championship`/`Match` finders through `current_section`, added `ApplicationController#verify_section_ownership!` helper, hardened `MatchesController#create`/`#update` against `championship_id` body-param spoofing): branch `security/scope-championship-match-finders` — merged (#1101). PR 3 (selection/day/training family): scoped `SelectionsController#index`/`#destroy`, `DaysController#update` (added `ApplicationController#verify_day_ownership!`, since `Day` belongs to a season-wide `Calendar` rather than a section — ownership is derived from having a match in `current_section`'s championships), `TrainingPresencesController` (all actions, training scoped via `Training.of_section`, target user scoped via `current_section.users`), and `MatchesController#selection` (user_id/team_id were unscoped, allowing selecting arbitrary users/teams onto a match); also widened `ApplicationController#catch404` to respond to `json`/`js` formats, since some of these endpoints are JSON/JS-only and previously 500'd instead of 404'ing. Branch: `security/scope-selection-day-training-finders` — merged (#1102). PR 4 (users/absences/participations/messages family): `UsersController#find_user_by_id` now scopes `@user` through `current_section.users` when a section context is present (was a plain `User.find`, so any coach could view/edit/delete profile data — including email/phone, used for account-takeover-via-password-reset — of a user from any other club, or add arbitrary users to their own section's roster); `#destroy`'s `group_id` branch now scopes through `current_section.groups`. `AbsencesController` reordered `set_user`/`set_absence` so `@user` is resolved first and scoped via `current_section.users`, and `@absence` is scoped via `@user.absences` (was `Absence.find` — any user could edit/delete/reassign any absence in the system). `MessagesController#set_channel` now scoped via `current_section.channels` (was `Channel.find` — any member could post/delete messages in another club's channel). `ParticipationsRenewalController#renew_players`/`#renew_coachs` now scope candidate users to actual previous-season members of the section (was `User.find` on raw ids — any coach could enroll arbitrary users as players/coaches of their section). `ChannelsController`/`ParticipationsController` were already correctly scoped. Branch: `security/scope-users-absences-messages-finders`. Remaining: un-nested routes cleanup (top-level `resources :days`/`:championships` bypass `current_section` entirely), low-severity self-referential match/training id scoping in `UsersController#match_availabilities`/`#training_presences` (a user can already only affect their own presence/availability rows, but can point them at matches/trainings outside their sections), then remove Pundit gem.
- **Priority**: 1
