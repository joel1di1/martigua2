...
- **Status**: done — batch-converted all 47 remaining HAML views to Slim by hand (the `haml2slim` gem turned hash-style attributes into inline text, so its output was unusable) and removed `haml-rails` from the Gemfile. Kept the legacy Bootstrap classes as-is (class migration is out of scope); used `div class="..."` Slim style per CLAUDE.md. Verified: every one of the 122 Slim templates compiles (`Slim::Template.new`), full suite passes without haml-rails, and the four converted mailer templates were smoke-rendered (`send_match_invitation`, `send_training_invitation`, `send_section_addition_to_existing_user`, `send_tig_mail_for_training`, plus devise `invitation_instructions` via `user.invite!`). Behavioral fix included: in the old `send_match_invitation`/`send_training_invitation` HAML, the "Dispo pour aucun"/"Présent à aucun" links had `user_token:` **outside** the `*_user_url(...)` call (it rendered as a dead HTML attribute), so those links carried no auth token; the token is now inside the URL like the "tous" links. Branch: `chore/p12-haml-to-slim` (stacked on P11). ERB views (16) not in scope of this item.
  ...
  **Follow-up (2026-09-03), branch `worktree-p12-finish-haml-to-slim`**: this item was not actually finished; the two loose ends below were found by re-auditing the code rather than trusting this status line. (1) `haml-rails` was never removed — searching the Gemfile's history for the dependency showed only the original 2014 addition and no removal commit, so it was still at `Gemfile:20`. (2) Because the gem was still installed, three *new* HAML mailer views rendered fine and nobody noticed them land after the migration: `_player_footer.html.haml`, `send_contact_email_welcome.html.haml` and `send_login_link.html.haml`, all added on 2026-09-03 by the relatives/invitation work (`5a218112`, `c5bd5173`). Converted all three to Slim by hand and removed `haml-rails` (`haml` and `haml-rails` are both gone from `Gemfile.lock`). One rendering fix along the way: HAML emitted a newline between a trailing `... ici :` / `... connexion :` text line and the following `%a`, which browsers collapse to a space; a plain Slim `|` block does not, so those two lines use Slim's trailing-whitespace marker `'` to keep the space before the link. Verified by rendering `send_login_link`, `send_contact_email_welcome` and the `player_footer` partial and inspecting the HTML. Added `spec/conventions_spec.rb` asserting `app/views/**/*.haml` is empty, so the convention is now enforced instead of re-migrated — confirmed it fails (naming the offending file) when a `.haml` view is present.
- **Priority**: 12
