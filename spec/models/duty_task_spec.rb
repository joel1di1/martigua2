...
RSpec.describe DutyTask do
  ...
  describe ".for_current_season" do
    let(:current_season_start) { Date.new(Date.today.year, 8, 1) }
    let(:current_season_end) { Date.new(Date.today.year + 1, 6, 30) }
    ...
    it "returns duty tasks for the current season" do
      ...
      other_season_tasks = [
        create(:duty_task, realised_at: current_season_start - 1.month),
        create(:duty_task, realised_at: current_season_end + 1.month),
        create(:duty_task, realised_at: current_season_start - 1.year),
        create(:duty_task, realised_at: current_season_end + 1.year)
      ]
