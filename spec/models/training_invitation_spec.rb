...
RSpec.describe TrainingInvitation, :type => :model do
  ...
  describe '#send_invitations_for_undecided_users' do
    ...
    let!(:training_availability) { create :training_availability, user: player_already_answered, training: training, available: [true, false].sample }
    let!(:training_availability_2) { create :training_availability, user: player_already_answered_with_nil, training: training, available: nil }
