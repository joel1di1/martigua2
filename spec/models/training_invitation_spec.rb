...
RSpec.describe TrainingInvitation, type: :model do
  ...
  describe '#send_invitations_for_undecided_users' do
    ...
    let!(:training_presence) { create :training_presence, user: player_already_answered, training:, is_present: [true, false].sample }
    let!(:training_presence_2) { create :training_presence, user: player_already_answered_with_nil, training:, is_present: nil }
